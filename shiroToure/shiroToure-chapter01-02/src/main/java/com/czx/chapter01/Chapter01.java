package com.czx.chapter01;
/**
 * 参考地址：http://blog.csdn.net/lhacker/article/details/10438387
 * 一 什么是shiro
 ...
 * 	SecurityManager
 * 	Subject的“幕后”推手是SecurityManager.Subject代表了当前用户的安全操作，
 * 	SecurityManager则管理所有用户的安全操作。它是Shiro框架的核心，充当“保护伞”，
 * 	引用了多个内部嵌套安全组件，它们形成了对象图。但是，一旦SecurityManager及其内部对象图配置好，
 * 	它就会退居幕后，应用开发人员几乎把他们的所有时间都花在Subject API调用上。

 *  Realms
 *  Shiro的第三个也是最后一个概念是Realm。Realm充当了Shiro与应用安全数据间的“桥梁”或者“连接器"。
 *  也就是说，当切实与像用户帐户这类安全相关数据进行交互，执行认证（登录）和授权（访问控制）时，
 *  Shiro会从应用配置的Realm中查找很多内容。
 *  
 *  
 *  
 * @author admin
